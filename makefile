...
CSRC_RECURSIVE_DIR := ./utest/Tests
CSRC_DIR := ./
CSRC_DIR += ./utest/cpputest/src/CppUTest
CSRC_DIR += ./utest/cpputest/src/CppUTestExt
CSRC_DIR += ./utest/cpputest/src/Platforms/Gcc
CSRC_FILE :=

#Directories to include. Directories can overlap but will only be passed to compiler if it has at least one .h inside
INC_RECURSIVE_DIR :=
INC_DIR := ./
INC_DIR += ./utest/cpputest/include
INC_DIR += ./utest/cpputest/include/CppUTest
...
#Normalize directories removing ./ in the beginning, and / at the end
BUILD_DIR := $(BUILD_DIR:./%=%)
BUILD_DIR := $(BUILD_DIR:%/=%)
...
CSRC_IGNORE += $(call rwildcard, $(CSRC_IGNORE_RECURSIVE_DIR:%/=%), *.cpp)
CSRC_IGNORE += $(foreach d, $(CSRC_IGNORE_DIR:%/=%), $(wildcard $(d)/*.cpp))
...
CSRC += $(call rwildcard, $(CSRC_RECURSIVE_DIR:%/=%), *.cpp)
CSRC += $(foreach d, $(CSRC_DIR:%/=%), $(wildcard $(d)/*.cpp))
CSRC += $(CSRC_FILE)
CSRC := $(CSRC:./%=%)
CSRC := $(call uniq, $(CSRC))
CSRC := $(CSRC:%=%)
...
#Second, script functions are executed (echo, gcc). When compiling, the console will print "Generating objlist" (see below), then "Linking" 
	@printf "Linking...\n"
	@g++ @$(BUILD_DIR)/objlist.list -o $(@) $(LDFLAGS)
...
#This is necessary later
	...
	@printf "Compiling $(<)\n"
	@gcc -c $(<) -o $(@) -MMD -MP $(INC_DIRS:%=-I%) $(CFLAGS) $(DEFINES)

#Do the same for cpp files
$(BUILD_DIR)/%.cpp.o: $$(subst .._/, ../, %.cpp)
...
#Compile .cpp file using -c flag to not link it. -MP and -MMD are used to create .d along with .o ; .d files create targets for each .o that depands on every .h inside the code
#This is necessary later

	@printf "Compiling $(<)\n"
	@g++ -c $(<) -o $(@) -MMD -MP $(INC_DIRS:%=-I%) $(CXXFLAGS) $(DEFINES)
